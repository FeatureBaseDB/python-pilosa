...
class Query(object):
    ...
    def __init__(self, *inputs):
        ...
        if hasattr(self, 'input_limit') and len(self.inputs) > self.input_limit:
            raise InvalidQuery("number of inputs ({0}) exceeds input limit ({1}) for {2} query".format(len(self.inputs), self.input_limit, self.__class__.__name__))

    def to_pql(self):
        return '{0}({1})'.format(self.__class__.__name__, ', '.join(subq.to_pql() for subq in self.inputs))
...
class SetBit(Query):
    ...
    def to_pql(self):
        return '{0}(id={1}, frame="{2}", profileID={3})'.format(self.__class__.__name__, self.id, self.frame, self.profile_id)
...
class SetBitmapAttrs(Query):
    ...
    def to_pql(self):
        attrs = ', '.join("{0}={1}".format(k,_escape_string_value(v)) for k,v in self.attrs.items())
        return 'SetBitmapAttrs(id={0}, frame="{1}", {2})'.format(self.id, self.frame, attrs)
...
class Bitmap(Query):
    ...
    def to_pql(self):
        return 'Bitmap(id={0}, frame="{1}")'.format(self.id, self.frame)
...
class SetProfileAttrs(Query):
    ...
    def to_pql(self):
        attrs = ', '.join("{0}={1}".format(k,_escape_string_value(v)) for k,v in self.attrs.items())
        return 'SetProfileAttrs(id={0}, {1})'.format(self.id, attrs)
